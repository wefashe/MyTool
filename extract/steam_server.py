...
async def send_message(ws, msg_type, body):
    ...
    header = bytearray(8)
    header[:4] = (int(msg_type) | PROTO_MASK).to_bytes(4, byteorder='little', signed=False)
...
async def handle_message(body):
    ...
    if msg_proto_buf_header['client_sessionid'] and msg_proto_buf_header['client_sessionid'] != client_session_id:
        print(f'Got new client session id {msg_proto_buf_header["client_sessionid"]}');
        client_session_id = msg_proto_buf_header['client_sessionid']
    eMsg = (rawEmsg & ~PROTO_MASK)  
    if eMsg != Multi:
        print(f'Receive: { eMsg} ({msg_proto_buf_header["target_job_name"]})')

    if msg_proto_buf_header['jobid_target'] and jobs[msg_proto_buf_header['jobid_target']]:
        pass
    if eMsg == CLIENT_LOG_ON_RESPONSE: # 751时 换服务器
        ...
        print(f'Received ClientLogOnResponse with result: {log_on_response["eresult"]}')
    elif eMsg == Multi:
        pass
    else:
        ...
        while True: # 断线重连
            try:
                ...
                async with websockets.connect(f'wss://{endpoint}/cmsocket/') as websocket:
                    ...
                    await send_message(websocket, CLIENT_HELLO, encoded_msg_client_hello)
